...
def enhancedFeatureExtractorDigit(datum):
    ...
    ##basicFeatureExtractorDigit
    """
    features = basicFeatureExtractorDigit(datum)
    digitHeight = range(DIGIT_DATUM_HEIGHT)
    digitWidth = range(DIGIT_DATUM_WIDTH)
    height = 0

    while height in digitHeight:
        pixelChanged = 0
        lastDatum = 0
        width = 0
        while width in digitWidth:
            currentDatum = datum.getPixel(width, height)
            if currentDatum > 1:
                features[(width, height)] = 1
            if currentDatum != lastDatum:
                pixelChanged += 1
                lastDatum = datum.getPixel(width, height)
            width = width + 1
        noloopy = 'noloopy' + str(height)
        loopy = 'loopy' + str(height)
        noloopx = 'noloopx' + str(width)
        loopx = 'loopx' + str(width)
        features[noloopx] = 0
        features[loopx] = 0
        features[noloopy] = 0
        features[loopy] = 0
...
def enhancedPacmanFeatures(state, action):
    ...
    foodPos = successorState.getFood()
    ghostPos = successorState.getGhostStates()
    pacmanPos = successorState.getPacmanPosition()

    ghostScore = 0.0
    for pos in ghostPos:
        dist = util.manhattanDistance(pacmanPos, pos.getPosition())
        if dist != 0:
            ghostScore = ghostScore + (1 / dist)

    foodScore = 0.0
    for pos in foodPos:
        foodScore = foodScore + (1 / util.manhattanDistance(pacmanPos, pos))
